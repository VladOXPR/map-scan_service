...
(function (global) {
    ...
    var CSS = [
        ...
        '.cuub-loc-modal-title { font-size: 18px; font-weight: 600; margin-bottom: 20px; }',
    ...
    function injectDom() {
        ...
        root.innerHTML = [
            ...
            '<div class="cuub-loc-modal cuub-hidden" id="cuubLocModal" role="dialog" aria-modal="true" aria-labelledby="cuubLocModalTitle">',
            '  <h2 class="cuub-loc-modal-title" id="cuubLocModalTitle">CUUB near you?</h2>',
            ...
            '    <button type="button" class="cuub-loc-btn cuub-loc-btn-primary" id="cuubLocBtnYes">Find</button>',
            '  </div>',
            '</div>'
        ].join('');
    ...
    // Works from cross-origin iframes (e.g. Framer) because we use a synthetic
    // <a target="_blank"> click — this lets the OS handle custom protocols
    // (maps://, google.navigation:) instead of trying to navigate the iframe
    // itself, which browsers silently block.
    function openDirectionsTo(latitude, longitude) {
        ...
        // Primary: synthetic anchor click. Uses _blank so it works inside
        // sandboxed/cross-origin iframes and correctly hands custom protocols
        // off to the OS.
        try {
        ...
        // Fallback 1: window.open in a new tab.
        try {
            var win = global.open(url, '_blank', 'noopener');
            if (win) return;
        } catch (_) { /* fall through */ }

        // Fallback 2: navigate the top window (works for user-initiated clicks
        // across origins); finally, last-resort same-frame navigation.
        try {
        ...
        try { global.location.href = url; } catch (_) { /* noop */ }
    }
    ...
    function attach(options) {
        ...
        var hideTriggerButton = !!options.hideTriggerButton;
        var disableAutoPrompt = !!options.disableAutoPrompt;
        if (!map || !mapboxgl) {
        ...
        function applyUserCoords(coords) {
            ...
            var stationOk = Number.isFinite(stationLng) && Number.isFinite(stationLat);

            // Measure the actual map container so padding can't exceed it
            // (fitBounds silently skips when padding >= container size, which
            // is easy to hit inside small Framer iframes).
            var container = map.getContainer ? map.getContainer() : null;
            var containerWidth = (container && container.clientWidth) || global.innerWidth || 1024;
            var containerHeight = (container && container.clientHeight) || global.innerHeight || 768;
            var isNarrow = containerWidth < 480;
            var isShort = containerHeight < 520;

            // Desired padding, then clamp so top+bottom <= ~70% of height, sides
            // likewise. Keeps fitBounds working on very small embeds.
            var desiredTop = isStickerPage
                ? (isNarrow ? 320 : 340)
                : (isShort ? 120 : (isNarrow ? 220 : 240));
            var desiredBottom = isShort ? 80 : (isNarrow ? 120 : 140);
            var desiredLeft = isNarrow ? 40 : 80;
            var desiredRight = isNarrow ? 40 : 80;

            var maxVertical = Math.max(40, Math.floor(containerHeight * 0.7));
            var maxHorizontal = Math.max(40, Math.floor(containerWidth * 0.7));
            var vSum = desiredTop + desiredBottom;
            if (vSum > maxVertical && vSum > 0) {
                var vScale = maxVertical / vSum;
                desiredTop = Math.floor(desiredTop * vScale);
                desiredBottom = Math.floor(desiredBottom * vScale);
            }
            var hSum = desiredLeft + desiredRight;
            if (hSum > maxHorizontal && hSum > 0) {
                var hScale = maxHorizontal / hSum;
            ...
            // Fallback: just fly to the user's location if fitBounds couldn't
            // be used (invalid station coords or thrown error).
            if (!fitted) {
                map.flyTo({
                    center: [coords.longitude, coords.latitude],
                    zoom: 15.2,
        ...
        // --- postMessage bridge (for external/parent triggers) ------------
        // Accept commands from a parent page (e.g. Framer) of shape:
        //   { source: 'cuub', type: 'findNearest' | 'requestLocation' | 'ping' }
        // We don't restrict event.origin because this page is designed to be
        // embedded on arbitrary parent origins; only local UI actions are fired.
        function sendToParent(message) {
            try {
                if (global.parent && global.parent !== global) {
                    global.parent.postMessage(message, '*');
                }
            } catch (_) { /* cross-origin access denied */ }
        }
        function onParentMessage(event) {
            var data = event && event.data;
            if (!data || typeof data !== 'object') return;
            if (data.source !== 'cuub' || typeof data.type !== 'string') return;
            switch (data.type) {
                case 'findNearest':
                    // Respects the same UX as the internal trigger: opens the
                    // permission prompt modal.
                    openModal();
                    break;
                case 'requestLocation':
                    // Skip the prompt and go straight to the OS permission.
                    requestLocation();
                    break;
                case 'ping':
                    sendToParent({ source: 'cuub', type: 'pong' });
                    break;
                default:
                    break;
            }
        }
        global.addEventListener('message', onParentMessage, false);

        // --- Wire events ---------------------------------------------------
        if (!hasGeolocation()) {
            if (triggerBtn) hide(triggerBtn);
            setTimeout(function () { showToast('Location unavailable — showing all stations.'); }, 600);
            sendToParent({ source: 'cuub', type: 'ready', geolocation: false });
        } else {
            if (triggerBtn) {
            ...
            // Auto-show once per session unless the page has opted out (e.g. a
            // parent Framer page owns the trigger).
            ...
            sendToParent({ source: 'cuub', type: 'ready', geolocation: true });
        }
